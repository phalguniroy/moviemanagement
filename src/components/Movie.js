...
const Movie = ({ title, year, poster, index, updateCheckedList, handleAddToListClick, watched, remove }) => {
    ...
    return (
        <div>
            <Card style={{ width: '700px' }} className="m-3" >
                <Card.Body>
                <div>
                <Card.Img style={{ height: '18rem', width: '400px' }} className='pl-4 pr-4' variant="top" src={poster} />
                <Form className="float-right">
                    <Form.Check onChange={(e) => { updateCheckedList(index, e.target.checked) }}  type="checkbox" className='m-3' />
                </Form>
                </div>
